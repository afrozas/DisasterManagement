...
class User(AbstractBaseUser):
    ...
    watching = models.ManyToManyField('self',
                                      ...
                                      through='Watch',
                                      through_fields=('watcher', 'watchee'),
                                      symmetrical=False,
                                      blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.prev_is_safe and self.is_safe:
            self.notify_watchers(f'{self.name} is safe!', 'Autogenerated mail')
            self.prev_is_safe = True
        super().save(*args, **kwargs)

    def notify_watchers(self, subject, message):
        datatuple = tuple((subject, message, None, [user.email])
                          for user in self.watchers.all())
        send_mass_mail(datatuple)


class Watch(models.Model):
    watchee = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='watchee')
